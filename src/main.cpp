...
// Naive O(n) linear search: scan ALL entries to find those matching a given IP
...
void displayPerformanceReport(const std::vector<TimingResult>& timings,
    ...
    // DSA comparison: HashMap O(1) vs Linear Search O(n)
    ...
    std::cout << "2. Sliding Window (Deque): O(n) burst detection vs O(n^2) nested loops\n";
    std::cout << "   BruteForce & ErrorSpike use monotonic time windows\n";
    std::cout << "3. Priority Queue (Max-Heap): O(log n) insert + O(n log n) extract\n";
    std::cout << "   Alerts ranked by severity without full re-sort\n";
    std::cout << "4. std::sort: O(n log n) guaranteed via IntroSort\n";
...
int main(int argc, char* argv[]) {
    ...
    if (entries.empty()) {
        std::cout << "[ERROR] No entries loaded. Exiting.\n";
        return 1;
    }

    // Run the analysis pipeline with per-stage timing
    ThreatAnalyzer analyzer;
    analyzer.analyze(entries);

    if (runPerformanceAnalysis) {
        // Re-run with granular timing for performance report
        ...
        // Stage 2: HashMap index build
        ThreatAnalyzer perfAnalyzer;
        auto s1 = std::chrono::high_resolution_clock::now();

        // Build index (we access it via getIndex after analyze)
        // Re-run buildIndex timing by analyzing
        ...
        // Measure HashMap index build separately
        {
        ...
        // Scoring
        {
            ...
            // Don't print again, just time it
